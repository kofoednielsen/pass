...
application state pending = do
    ...
    WS.withPingThread conn 30 (return ()) $ do
      -- When a client is succesfully connected, we read the first message. This
      -- should be in the format of "Hi! I am Jasper", where Jasper is the
      -- requested username.
      ...
      case msg of
        ...
        _   | not (prefix `T.isPrefixOf` msg) ->
            ...
            -- All is right! We're going to allow the client, but for safety
            -- reasons we *first* setup a `disconnect` function that will be run
            -- when the connection is closed.
            | otherwise -> flip finally disconnect $ do
                -- We send a "Welcome!", according to our own little
                -- protocol. We add the client to the list and broadcast the
                -- fact that he has joined. Then, we give control to the 'talk'
                -- function.
